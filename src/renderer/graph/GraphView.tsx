...
export default function GraphView(){
 ...
 return <section className="graph-view">
  ...
  <div className="graph-canvas" onWheel={e=>{e.preventDefault();setZoom(z=>Math.max(.65,Math.min(1.8,z+(e.deltaY<0?.08:-.08))))}}>
  ...
  {selected&&<aside className="graph-details"><button onClick={()=>setSelected(null)}>×</button><strong>{selected.label}</strong><span>{selected.kind}</span><small>Status: {selected.status||"unknown"}</small><p>Origem visual temporária. A fonte de verdade será o Potencia Runtime.</p></aside>}
 </section>;
}
