...
function targetFor(agent:OfficeAgent){
 ...
 for(let i=0;i<12;i++){const p=clampPoint({x:area.x+Math.random()*area.width,y:area.y+Math.random()*area.height},area);if(!blocked(p.x,p.y))return p;}
...
export default function OfficeView(){
 ...
 useEffect(()=>{
  ...
  const timer=window.setInterval(()=>{
   ...
   setAgents(current=>current.map(agent=>{
    ...
    const nx=agent.x+(dx/d)*1.25,ny=agent.y+(dy/d)*1.25;
 ...
 useEffect(()=>{
  const d=officeDesks.reduce<{id:string;dist:number}|null>((best,d)=>{
   const dist=Math.hypot(d.x+33-player.x,d.y+30-player.y);
   return dist<55&&(!best||dist<best.dist)?{id:d.id,dist}:best;
  },null);
  setNearDesk(d?.id||null);
 },[player]);

 useEffect(()=>{
  if(!nearDesk)return;
  const onKey=(e:KeyboardEvent)=>{if(e.key.toLowerCase()!=="e")return;setSeated(v=>!v);};
 ...
 return <section className="office-view">
  ...
  <div className="office-viewport"><div className="office-world" style={{transform:`translate(-${camera.x}px,-${camera.y}px)`}}>
   ...
   {officeDesks.map(d=><div key={d.id} className={`desk ${agents.some(a=>a.deskId===d.id)?"desk--occupied":""}`} style={{left:d.x,top:d.y}}><div className="monitor"/><div className="keyboard"/><div className="chair"/><span className="desk-label">{agents.some(a=>a.deskId===d.id)?"ocupada":""}</span></div>)}
